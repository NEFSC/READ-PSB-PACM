...
projects_no_detections <- setdiff(unique(df_projects_all$id), unique(df_detects_all$id))
# cat("excluding projects with no detection data (n = {length(projects_no_detections)})")
...
df_projects <- df_projects_all %>% 
  filter(!id %in% c(projects_no_detections, projects_invalid_latlon))
...
df_detects <- df_detects_all %>% 
  ...
  select(dataset, species, id, date, presence)

# df_detects %>% 
#   janitor::tabyl(species, call_type)
...
df_gaps <- df_detects %>% 
  select(id, date, species, presence) %>% 
  as_tsibble(key = c(id, species), index = date) %>% 
  count_gaps()
if (nrow(df_gaps) > 0) {
  cat(glue("detected {nrow(df_gaps)} gaps"))
}
