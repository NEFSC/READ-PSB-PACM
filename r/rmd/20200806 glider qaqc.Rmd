---
title: "QAQC Glider Dataset (v. 2020-08-06)"
...
df_meta_csv <- read_csv(
  "~/Dropbox/Work/nefsc/transfers/20200806 - glider data/Glider_metadata_2020-08-06.csv",
  col_types = cols(
...
stopifnot(length(no_detections) == 0)
```
...
The `detection_method` column is the same for all species.
...
Each `detection_method` method has a unique `protocol_reference`.
...
Each deployment should contain unique values for `analysis_period_start_datetime` (no duplicate analysis periods).
...
The range of analysis start/end timestamps is always within the monitoring period for each deployment.
