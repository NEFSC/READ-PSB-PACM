# JASCO_20250326 - legacy PACM_20240820 submission, converted to PARS (T3.2, AD-12).
#
...
dir <- "data-raw/pars/JASCO_20250326"

raw_metadata <- read_csv(
  list.files(file.path(dir, "raw"), pattern = "_METADATA_", full.names = TRUE),
  id = "$file",
  col_types = cols(.default = col_character())
)
raw_detections <- read_csv(
  list.files(file.path(dir, "raw"), pattern = "_DETECTIONDATA_", full.names = TRUE),
  id = "$file",
  col_types = cols(.default = col_character())
)
...
metadata <- raw_metadata |> 
  mutate(
    UNIQUE_ID = case_when(
      str_detect(`$file`, "_HF") ~ glue("{UNIQUE_ID}-HF"),
      str_detect(`$file`, "_LF") ~ glue("{UNIQUE_ID}-LF"),
      TRUE ~ UNIQUE_ID
    ),
...
analyses <- raw_detections |> 
  ...
  mutate(
    UNIQUE_ID = str_replace(UNIQUE_ID, "_ NA", "_AMAR532"),
    UNIQUE_ID = case_when(
      str_detect(`$file`, "_HF") ~ glue("{UNIQUE_ID}-HF"),
      str_detect(`$file`, "_LF") ~ glue("{UNIQUE_ID}-LF"),
      TRUE ~ UNIQUE_ID
...
detections <- analyses |> 
  ...
  select(-DATE) |> 
  filter(
    !is.na(SPECIES_CODE) # one row had missing species and start/end analysis timestamps, comment says manual detection, just dropping it
  )
