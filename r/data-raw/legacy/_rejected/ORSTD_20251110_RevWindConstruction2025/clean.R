...
dir <- "data-raw/legacy/_rejected/ORSTD_20251110_RevWindConstruction2025"


# thayer mahan -----------------------------------------------------------
# detectiondata deployment_code ambiguous
...
unique(raw_detections_tm$deployment_code)
...
raw_metadata <- list.files(file.path(dir, "raw"), pattern = "METADATA[.]xlsx", full.names = TRUE, recursive = TRUE) |> 
  ...
  mutate(
    ...
    UNIQUE_ID = glue("{UNIQUE_ID}_2025")
  )
tabyl(raw_metadata, UNIQUE_ID)

raw_detections <- list.files(file.path(dir, "raw"), pattern = "DETECTIONDATA[.]xlsx", full.names = TRUE, recursive = TRUE) |> 
  map_dfr(function (x) {
    x_cols <- readxl::read_excel(x, col_types = "text")
    x_coltypes <- rep("guess", ncol(x_cols))
    x_coltypes[which(names(x_cols) == "CALL_TYPE_CODE")] <- "text"
    readxl::read_excel(
      x,
      col_types = x_coltypes
    ) |> 
      remove_empty(which = c("rows", "cols")) |> 
      mutate(
        `$file` = x,
...
tabyl(raw_detections, UNIQUE_ID)
...
analyses <- raw_detections |> 
  ...
  filter(!is.na(UNIQUE_ID)) |> 
  mutate(
    # across(c(ANALYSIS_PERIOD_START_DATETIME, ANALYSIS_PERIOD_END_DATETIME), excel_numeric_to_date),
